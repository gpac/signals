...
unittest("audio converter: interleaved to planar to interleaved") {
	...
	ConnectPin(soundGen->getOutputPin(0), comparator.get(), &Utils::PcmComparator::pushOriginal);
	...
	ConnectPinToModule(soundGen->getOutputPin(0), converter1);
	ConnectPinToModule(converter1->getOutputPin(0), converter2);
	ConnectPin(converter2->getOutputPin(0), comparator.get(), &Utils::PcmComparator::pushOther);
...
unittest("audio converter: dynamic formats") {
	...
	ConnectPinToModule(soundGen->getOutputPin(0), converter);
	ConnectPinToModule(converter->getOutputPin(0), recorder);
	...
	converter->flush();
	converter->getOutputPin(0)->getSignal().disconnect(0);
...
unittest("video converter: pass-through") {
...

	{
		...
		ConnectPin(convert->getOutputPin(0), onFrame);
