...
unittest("render: A/V sync, one thread") {
	...
	ConnectOutputToModule(videoGen->getOutput(0), videoRender);

	auto soundGen = uptr(new In::SoundGenerator);
	auto soundRender = uptr(new Render::SDLAudio(clock.get()));
	ConnectOutputToModule(soundGen->getOutput(0), soundRender);
...
unittest("render: dynamic resolution") {
	...
	std::shared_ptr<Data> pic1 = uptr(new PictureYUV420P(Resolution(128, 64)));
	pic1->setTime(1000);
	videoRender->process(pic1);

	std::shared_ptr<Data> pic2 = uptr(new PictureYUV420P(Resolution(64, 256)));
	pic2->setTime(2000);
