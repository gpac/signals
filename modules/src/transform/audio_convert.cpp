...
AudioConvert::AudioConvert(AudioSampleFormat srcFmt, AudioLayout srcChannelLayout, uint32_t srcSampleRate, AudioStruct srcStruct,
	...
	: srcAudioCfg(new PcmFormat(srcSampleRate, srcChannelLayout, srcFmt, srcStruct)),
	  dstAudioCfg(new PcmFormat(dstSampleRate, dstChannelLayout, dstFmt, dstStruct)),
	...
	libavAudioCtxConvertLibav(srcAudioCfg.get(), aFrame->get()->sample_rate, aFrame->get()->format, aFrame->get()->channels, aFrame->get()->channel_layout);
	libavAudioCtxConvertLibav(dstAudioCfg.get(), avDstSampleRate, (int&)avDstFmt, avDstNumChannels, avDstChannelLayout);
...
void AudioConvert::process(std::shared_ptr<Data> data) {
	...
	if (audioData->getFormat() != *srcAudioCfg)
